        ...
        default:
            ...
            for (i = 0; i < nfds; i++) {
                if (events[i].data.fd == soc) {
                    ...
                    /* 接続受付 */
                    if ((acc = accept(soc, (struct sockaddr *) &from, &flen))
		        == -1) {
                        if(errno!=EINTR){
                            perror("accept");
                        }
			...
			= recv(g_queue[qi].data[g_queue[qi].last].acc,
				...
				(void) close(epollfd);
                        ...
                        default:
                            ...
                            fprintf(stderr, "locked accept loop %d\n", qi);
                            ...
                            fprintf(stderr, "unlocked accept loop %d\n", qi);
                            break;
                    }
                }
            }
...
send_thread(void *arg)
{
    ...
    for (;;) {
        (void) pthread_mutex_lock(&g_queue[qi].mutex);
        fprintf(stderr, "locked send_thread, %d\n", qi);
        if(g_queue[qi].last != g_queue[qi].front) {
            ...
            (void) pthread_mutex_unlock(&g_queue[qi].mutex);
            fprintf(stderr, "unlocked send_thread %d\n", qi);
            continue;
        }
        /* 文字列化・表示 */
